...
- **深圳天策科技顾问有限公司，技术部门，技术总监，2020.05 ~ 2020.10**

  这是我和几个伙伴成立的创业公司，主要的业务是为企业提供技术支持。我主要是负责公司所有项目的技术规划，参与公司发展战略和计划的制定，组织和管理技术团队，积极配合其它部门提供技术支持。
...
- **Godzpeed(客户项目)**
    ...
    > 使用到: C++、Koa.js、Mongodb、Taro、MySQL

    Godzpeed 是一家做模拟赛车游戏的公司，使用硬件设备模拟真实的赛车环境。我在该项目中主要负责微信小程序、后端程序、和赛车游戏模组的技术开发和管理工作。

    - 客户的盈利模式是按计时付费的模式，因此需要微信小程序提供支付和扫码的能力。小程序的架构使用了类 *React* 的 *Taro* 框架
    - 虽然支付能力搞定了，但还需要实现游戏内计时的功能。游戏本体是基于 *rFactor2* 的。我通过 *C++* 编写模组的形式将数据与云端同步，打通了与前端程序的连接
    - 后端使用 *Node.js* 的 *Koa.js* 框架，数据库选用了 *Mongodb* ，由于项目有 *IM* 功能，出于缓存和分布式的考虑，使用了 *Redis* 集群
    - 由于 *Mongodb* 老版本不支持传统的事务特性，为了保证支付模块的原子性，该模块改用了 *MySQL* 进行存储
    
    由于团队经验丰富，该项目从立项到交付仅用了 3 个月

- **荣阳铝业打卡程序(客户项目)**
 
   > 使用到: Cordova、Vue、Nodejs
  
   荣阳铝业是一家传统的实业公司，他们需要在部分工序上设置打卡环节。我在项目中主要负责打卡设备app的技术开发和管理工作。

   - 打卡设备实际上一台带扫码功能经过定制化的 *Android* 系统的移动设备。由于无体积限制，因此使用了 *Cordova* 开发
   - 后端使用 *Koa.js* + *Mongodb*
   - 该项目 2 个月完成了交付
...
- **Utomarket数字货币交易平台**
  ...
  - 开发过程中，前后端对于同一需求的复杂度存在开发工时的差异，这导致了开发进度不同步。为了解决前后端依赖问题，我们在新迭代开始前会先协同制定接口规范，然后使用 yapi 平台编写接口用例；这样，前端就可以在开发环境中使用yapi平台伪造的 *Mock* 数据而不依赖后端接口，实现前端先行。
